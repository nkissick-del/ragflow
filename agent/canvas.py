...
class Graph:
    ...
    def load(self):
        ...
        for k, cpn in self.components.items():
            ...
            cls = component_class(cpn["obj"]["component_name"] + "Param")
            if not cls:
                raise ValueError(self.get_component_name(k) + f": Component parameter class {cpn['obj']['component_name'] + 'Param'} not found")
            param = cls()
            ...
            cls = component_class(cpn["obj"]["component_name"])
            if not cls:
                raise ValueError(self.get_component_name(k) + f": Component class {cpn['obj']['component_name']} not found")
            cpn["obj"] = cls(self, k, param)
...
class Canvas(Graph):
    def __init__(self, dsl: str, tenant_id=None, task_id=None, canvas_id=None):
        self.globals = {"sys.query": "", "sys.user_id": tenant_id, "sys.conversation_turns": 0, "sys.files": [], "sys.history": []}
        self.variables = {}
    ...
    def load(self):
        ...
        if "globals" in self.dsl:
            ...
            self.globals = {"sys.query": "", "sys.user_id": "", "sys.conversation_turns": 0, "sys.files": [], "sys.history": []}
