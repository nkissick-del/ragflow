...
from rag.prompts.generator import next_step_async, COMPLETE_TASK, citation_prompt, kb_prompt, citation_plus, full_question, message_fit_in, structured_output_prompt
...
class Agent(LLM, ToolBase):
    ...
    def _load_tool_obj(self, cpn: dict) -> object:
        ...
        cls = component_class(tool_name + "Param")
        if not cls:
            raise ValueError(f"Tool parameter class {tool_name + 'Param'} not found")
        param = cls()
        ...
        cls = component_class(cpn["component_name"])
        if not cls:
            raise ValueError(f"Component class {cpn['component_name']} not found")
        return cls(self._canvas, cpn_id, param)
