...
class CitationService:
    ...
    def insert_citations(self, answer, chunks, chunk_v, embd_mdl, tkweight=0.1, vtweight=0.9):
        ...
        for i, t in enumerate(pieces):
            if len(t) < self.MIN_PIECE_LENGTH:
                continue
            idx.append(i)
            pieces_.append(t)
        ...
        cites = {}
        thr = self.INITIAL_SIM_THRESHOLD
        while thr > self.MIN_SIM_THRESHOLD and len(cites.keys()) == 0 and pieces_ and chunks_tks:
            for i, a in enumerate(pieces_):
                sim, tksim, vtsim = self.qryr.hybrid_similarity(ans_v[i], local_chunk_v, rag_tokenizer.tokenize(self.qryr.rmWWW(pieces_[i])).split(), chunks_tks, tkweight, vtweight)
                mx = np.max(sim) * self.MAX_SIM_SCALE
