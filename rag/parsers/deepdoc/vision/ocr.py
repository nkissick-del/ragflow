...
def run_with_retries(predictor_call, input_dict, run_options, max_retries=3, delay_backoff=1):
    for i in range(max_retries):
        try:
            return predictor_call(None, input_dict, run_options)
        except Exception as e:
            if i >= max_retries - 1:
                raise e
            time.sleep(delay_backoff * (2**i))
...
def load_model(model_dir, nm, device_id: int | None = None):
    ...
    def cuda_is_available(device_id=None):
        try:
            import torch
    ...
    if cuda_is_available(device_id):
        ...
        run_options.add_run_config_entry("memory.enable_memory_arena_shrinkage", "cuda")
    ...
    with _model_cache_lock:
        if model_cached_tag not in loaded_models:
            loaded_models[model_cached_tag] = loaded_model
        else:
            loaded_model = loaded_models[model_cached_tag]
...
class TextRecognizer:
    ...
    def __call__(self, img_list):
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            outputs = run_with_retries(self.predictor.run, input_dict, self.run_options)
            preds = outputs[0]
...
class OCR:
    def __init__(self, model_dir=None):
        ...
        if not model_dir:
            ...
            except (ImportError, OSError, IOError, RepositoryNotFoundError, RevisionNotFoundError) as e:
                logging.warning(f"Failed to load OCR models from local path, attempting snapshot_download: {str(e)}")
    ...
    def get_rotate_crop_image(self, img, points, device_id=0):
        ...
        if dst_img_height * 1.0 / dst_img_width >= 1.5:
            ...
            rec_result = self.text_recognizer[device_id]([dst_img])
            ...
            rec_result = self.text_recognizer[device_id]([rotated_cw])
            ...
            rec_result = self.text_recognizer[device_id]([rotated_ccw])
    ...
    def detect(self, img, device_id: int | None = None):
        ...
        return list(zip(self.sorted_boxes(dt_boxes), [("", 0) for _ in range(len(dt_boxes))]))
    ...
    def __call__(self, img, device_id=0):
        ...
        for bno in range(len(dt_boxes)):
            tmp_box = dt_boxes[bno].copy()
            img_crop = self.get_rotate_crop_image(ori_im, tmp_box, device_id)
