...
class Pad:
    ...
    def __call__(self, data):
        ...
        if self.size:
            ...
            assert img_h <= resize_h2 and img_w <= resize_w2, "(h, w) of target size should be greater than or equal to (img_h, img_w)"
        else:
...
class E2EResizeForTest:
    def __init__(self, **kwargs):
        ...
        if "max_side_len" not in kwargs or "valid_set" not in kwargs:
            raise ValueError("E2EResizeForTest missing 'max_side_len' or 'valid_set' in kwargs")
...
class KieResize:
    ...
    def __call__(self, data):
        ...
        resize_points = self.resize_boxes(im_resized, points, scale_factor)
        data["ori_image"] = img
