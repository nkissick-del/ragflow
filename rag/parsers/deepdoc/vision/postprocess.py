...
class DBPostProcess:
    ...
    def box_score_slow(self, bitmap, contour):
        ...
        xmin = np.clip(np.floor(np.min(contour[:, 0])).astype(int), 0, w - 1)
        xmax = np.clip(np.ceil(np.max(contour[:, 0])).astype(int), 0, w - 1)
        ymin = np.clip(np.floor(np.min(contour[:, 1])).astype(int), 0, h - 1)
        ymax = np.clip(np.ceil(np.max(contour[:, 1])).astype(int), 0, h - 1)
    ...
    def decode(self, text_index, text_prob=None, is_remove_duplicate=False):
        ...
        for batch_idx in range(batch_size):
            ...
            if text_prob is not None:
                ...
                conf_list = [1] * max(1, len(char_list))
            if len(conf_list) == 0:
                conf_list = [0]
