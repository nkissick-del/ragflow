...
class SemanticChunk:
    ...
    text: str
    header_path: List[str]  # e.g., ["Introduction", "Background"]
...
class Semantic:
    ...
    def _parse_with_headers(content: str, chunk_token_num: int, overlap_percent: int, is_english: bool = True) -> List[SemanticChunk]:
        ...
        def get_header_path() -> List[str]:
            """Build header path from stack as list of strings."""
            return [h[1] for h in header_stack]

        def emit_chunk(text: str, header_path: List[str]):
...
# Module-level chunk function for task_executor compatibility
# This wraps the Semantic.chunk() method to match the signature expected by FACTORY
def chunk(filename, binary=None, from_page=0, to_page=100000, lang="Chinese", callback=None, **kwargs):
    """
    Module-level chunk function for task_executor.py FACTORY compatibility.

    This function adapts the Semantic template to the standard RAGFlow chunking interface.
    It parses the file using appropriate parsers and then applies semantic chunking.

    Args:
        filename: Name of the file to process
        binary: Binary content of the file
        from_page: Starting page (for PDFs)
        to_page: Ending page (for PDFs)
    ...
    if callback:
        callback(0.1, "[Semantic] Starting semantic chunking...")

    # Determine the layout recognizer to use (prefer Docling for semantic)
    layout_recognizer, parser_model_name = normalize_layout_recognizer(parser_config.get("layout_recognizer", "Docling"))

    # Get the file content using appropriate parser
    content = ""
    ...
    if re.search(r"\.(txt|md|markdown|mdx)$", filename, re.IGNORECASE):
    ...
    elif re.search(r"\.(pdf|docx|doc|html|htm)$", filename, re.IGNORECASE):
        # Use Docling for semantic-aware parsing
        try:
            sections, tables, parser = by_docling(
                filename=filename,
                binary=binary,
                from_page=from_page,
                to_page=to_page,
                lang=lang,
                callback=callback,
                use_semantic_chunking=True,  # Request structured markdown
                **kwargs,
            )
            ...
            logging.warning(f"[Semantic] Docling parsing failed: {e}, falling back to DeepDOC")
            if callback:
                callback(0.2, f"[Semantic] Docling failed ({e}), using DeepDOC...")
            sections, tables, section_images, pdf_parser = by_deepdoc(
                filename=filename,
                binary=binary,
                from_page=from_page,
                to_page=to_page,
        ...
        raise NotImplementedError(f"[Semantic] File type not supported: {filename}")

    if not content.strip():
        if callback:
            callback(0.99, "[Semantic] No content parsed from file")
        return []

    # Create StandardizedDocument and process with Semantic template
    standardized = StandardizedDocument(_content=content, metadata={"parser": "semantic"})

    # Call Semantic.chunk with the standardized document
    return Semantic.chunk(
        filename=filename,
        standardized_doc=standardized,
