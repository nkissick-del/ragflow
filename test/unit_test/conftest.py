...
# 0. Mock beartype FIRST (type checking library used by deepdoc) - UNMOCKED
...
# 0d. Mock cv2 (OpenCV library used by deepdoc vision) - UNMOCKED
...
# 0e. Mock onnxruntime (used by deepdoc vision) - UNMOCKED
...
# 0o. Mock werkzeug - REMOVED (Needed by Quart/Flask)
# sys.modules["werkzeug"] = MagicMock()
# sys.modules["werkzeug.security"] = MagicMock()

# 0p. Mock playhouse - UNMOCKED
...
# 1. Mock opendal FIRST (Rust-based storage library, hard to install on Mac) - UNMOCKED
# mock_opendal = MagicMock()
# sys.modules["opendal"] = mock_opendal

# 2. Mock infinity FIRST (RAGFlow's tokenizer library) - UNMOCKED
...
# 4. Mock XGBoost to avoid libomp dependency issues on Mac - UNMOCKED
# mock_xgb = MagicMock()
