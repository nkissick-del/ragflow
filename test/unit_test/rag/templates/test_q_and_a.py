...
class TestQAndA(unittest.TestCase):
    """Test cases for the q_and_a.chunk function."""
    ...
    def test_chunk_excel(self):
        ...
        with patch("rag.templates.q_and_a.Excel") as MockParser:
            ...
            res = q_and_a.chunk("test.xlsx", b"content", callback=self.callback)
    ...
    def test_chunk_txt(self):
        ...
        with patch("rag.templates.q_and_a.get_text") as mock_get:
            mock_get.return_value = "Question,Answer"

            res = q_and_a.chunk(filename, binary, callback=self.callback)
            ...
            self.assertTrue("问题：" in res[0]["content_with_weight"] or "Question:" in res[0]["content_with_weight"])
    ...
    def test_unsupported_file_format(self):
        ...
        with self.assertRaises(NotImplementedError) as context:
            q_and_a.chunk("test.unsupported", b"content", callback=self.callback)
    ...
    def test_rmprefix_function(self):
        ...
        for input_text, expected in test_cases:
            with self.subTest(input=input_text):
                result = q_and_a.rmPrefix(input_text)
    ...
    def test_english_language_flag(self):
        ...
        with patch("rag.templates.q_and_a.get_text") as mock_get:
            ...
            # Test with English
            res_en = q_and_a.chunk("test.txt", b"data", lang="English", callback=self.callback)
            ...
            mock_get.return_value = "Question,Answer"
            res_zh = q_and_a.chunk("test.txt", b"data", lang="Chinese", callback=self.callback)
...
class TestMdQuestionLevel(unittest.TestCase):
    ...
    def test_heading_levels(self):
        ...
        for input_text, expected in test_cases:
            with self.subTest(input=input_text):
                result = q_and_a.mdQuestionLevel(input_text)
                self.assertEqual(result, expected)

    def test_no_heading(self):
        """Test that non-heading text returns level 0."""
        result = q_and_a.mdQuestionLevel("Regular text")
        self.assertEqual(result[0], 0)

    def test_empty_heading(self):
        """Test heading with no text after hashes."""
        result = q_and_a.mdQuestionLevel("### ")
